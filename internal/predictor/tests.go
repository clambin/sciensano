...
func ForecastTests(tests []sciensano.Test) (forecast []sciensano.Test, score float64, err error) {
	...
	go forecastSamples(input[0], input[1], ForecastBatches*BatchSize, "test", totalTests)
	go forecastSamples(input[1], input[0], ForecastBatches*BatchSize, "test", positiveTests)
	...
	for i := 0; i < ForecastBatches*BatchSize; i++ {
		...
		forecast = append(forecast, sciensano.Test{
			Timestamp: end,
			Total:     int(math.Max(0.0, <-totalTests)),
			Positive:  int(math.Max(0.0, <-positiveTests)),
		})
	}
...
func buildTestsInput(tests []sciensano.Test) (output [][]float64) {
	...
	for _, test := range tests[len(tests)-HistoryBatches*BatchSize:] {
